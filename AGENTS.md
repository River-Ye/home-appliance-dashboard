...
- 網站由現有 37 類資料產生 37 個 `/categories/<id>/` 靜態分類指南頁；不建立 1149 個重複商品事實的薄內容頁。
...
- 特殊分類目前數量：
  ...
  - `電腦螢幕`：63 筆，需涵蓋主流品牌、不同用途帶與 34/40/45/49/57 吋寬螢幕。
...
- `npm run check:geo`：37 個分類頁、1149 款靜態型號索引、metadata、結構化資料、首頁分類入口、sitemap、llms、六份公開證據檔、Pages artifact、38 個 canonical URL 的 IndexNow contract 與產生結果均無漂移。

...
- 網站由現有 30 類資料產生 30 個 `/categories/<id>/` 靜態分類指南頁；不建立 914 個重複商品事實的薄內容頁。
...
- `npm run check:geo`：30 個分類頁、914 款靜態型號索引、metadata、結構化資料、首頁分類入口、sitemap、llms、六份公開證據檔、Pages artifact、31 個 canonical URL 的 IndexNow contract 與產生結果均無漂移。

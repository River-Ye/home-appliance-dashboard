...
- 頁面目前標示主要資料擷取日為 2026-07-21；最新一輪於 2026-07-21 12:38（台灣時間；2026-07-21 04:38 UTC）以 666 筆基準清單全量重查價格、購買連結、最終 668 張圖片、438 筆 `found` 史低來源、匯率與原廠停產狀態。台灣商品優先台灣可信通路，外幣商品統一使用 2026-07-21 00:02 UTC 批次換算。
...
- 16 筆 PChome Qty 0 只列追蹤，沒有把缺貨價寫回；48 筆史低頁面本輪無法穩定重現或型號未能自動確認，保留原證據並列入稽核報告，未以單次錯誤臆測失效。沒有找到可直接套用的品牌官方明示停產證據。
- ExchangeRate-API 更新至 2026-07-21 00:02 UTC，所有外幣商品以同一批次重新換算；USD/TWD 為 32.327985。完整逐筆結果保存於 `catalog_maintenance_2026-07-21.json`。

...
- 特殊分類目前數量：
  ...
  - `Soundbar`：26 筆，需涵蓋不同聲道、擴充方式與空間需求。
  ...
  - `冰箱`：23 筆、`洗衣機`：23 筆、`烘衣機`：21 筆、`洗烘衣機(多功能型)`：24 筆，需排除配件、耗材、福利品、展示機、箱損品、組合包誤判與單功能錯類，且每筆需標示機身尺寸。
...
- 頁面目前標示主要資料擷取日為 2026-07-22；最新一輪於 2026-07-22 07:57（台灣時間）以 666 筆基準清單全量查核 400 筆 PChome API、266 筆其他購買連結、611 次圖片回應、437 筆 `found` 史低來源、匯率與停產字樣候選。台灣商品優先台灣可信通路，外幣商品才使用海外通路；好市多補充資料查核日同步為 2026-07-22。
- 更新 77 筆 PChome exact-model 現貨價（2 筆降價、75 筆漲價），未更換圖片連結；17 筆 PChome Qty 0 只保留追蹤。購買連結 6 筆、圖片 0 筆、史低來源 46 筆未能在本輪取得成功回應，均保留原資料並列入後續人工覆核，不以單次網路錯誤判定停產。
- 偵測到 4 筆頁面含停產字樣候選，但本輪未自動移除，需以原廠明確證據人工覆核；本輪未找到同時具備可信新品現貨、exact-model、公開售價、圖片、規格與售後證據而需立即納入的新產品。最終共 666 筆、共 25 類，且各類皆至少 20 筆。
- 最終史低狀態為 437 筆 `found`、229 筆 `not_found`。ExchangeRate-API 更新至 2026-07-21 00:02 UTC，17 筆外幣商品已依同一 USD 批次重算；USD/TWD 為 32.327985。
- 前一輪頁面標示主要資料擷取日為 2026-07-21；最新一輪於 2026-07-21 08:48（台灣時間；2026-07-21 00:48 UTC）以 666 筆基準清單全量重查價格、購買連結、666 張圖片、437 筆 `found` 史低來源、匯率與原廠停產狀態。台灣商品優先台灣可信通路，17 筆海外商品才比較海外通路；好市多補充資料查核日同步為 2026-07-21，目前好市多/Costco 候選共 64 筆，每類最多 3 筆。
- 本輪更新 79 筆既有商品現價（10 筆降價、69 筆漲價），並將 Marshall Heston 60 的失效 Best Buy 頁改為同型號 Marshall US 官方現貨頁；圖片連結均不需更換。17 筆 PChome Qty 0 與 Costco 明示缺貨價只保留追蹤，沒有寫回。既有 437 筆 `found` 史低來源沒有新低或失效，Sony HT-S40R 與 HT-S100F 則改採各自 exact-model LBJ 歷史曲線。
- 本輪移除與既有項目重複的 Loctek F2 120x60 黑色 variant，並納入 BenQ 台灣官方現貨、規格、上市月份、史低與 exact-model 負評覆核證據完整的 PD2732U 專業螢幕；升降桌為 22 筆、電腦螢幕為 55 筆，最終共 666 筆且所有分類仍至少 20 筆。未找到新的原廠停產證據；Acerpure RP722-12W、Dreame MF10 與 Dyson HJ10 仍因預購／尚未正式出貨而保留追蹤。
- 666 張商品圖片皆可讀取，最終 437 筆 `found`、229 筆 `not_found`。ExchangeRate-API 更新至 2026-07-20 00:02 UTC，17 筆外幣商品已依同一 USD 批次重算；USD/TWD 為 32.396939。
- 前一輪於 2026-07-20 06:27（台灣時間；2026-07-19 22:27 UTC）以 667 筆基準清單全量重查價格、購買連結、667 張圖片、438 筆 `found` 史低來源、匯率與原廠停產狀態。台灣商品優先台灣可信通路，17 筆海外商品才比較海外通路；好市多補充資料查核日同步為 2026-07-20，目前好市多/Costco 候選共 64 筆，每類最多 3 筆。
- 本輪更新 27 筆既有商品現價（9 筆降價、18 筆漲價），購買連結與圖片連結均不需更換；Circulon A1 26cm 的現價降至既有史低 NT$2,159，其餘既有史低沒有新低或失效證據。16 筆 PChome Qty 0 只保留追蹤，沒有把缺貨價寫回。
- BenQ 台灣官方購買頁明示 MOBIUZ EX271Q「本產品已停產」，因此移除該商品與對應研究紀錄；電腦螢幕仍有 54 筆，最終共 666 筆且所有分類仍至少 20 筆。本輪未找到證據完整且需立即納入的新產品；Acerpure RP722-12W 與 Dreame MF10 仍因預購／尚未正式出貨而保留追蹤。
- 667 張原始商品圖片皆可讀取；移除 EX271Q 後最終 437 筆 `found`、229 筆 `not_found`。ExchangeRate-API 更新至 2026-07-19 00:02 UTC，17 筆外幣商品已依新批次重算。
- 前一輪於 2026-07-19 06:27（台灣時間；2026-07-18 22:27 UTC）以 666 筆基準清單全量重查價格、購買連結、667 張圖片、438 筆 `found` 史低來源、匯率與原廠停產狀態。台灣商品優先台灣可信通路，17 筆海外商品才比較海外通路；好市多補充資料查核日同步為 2026-07-19，目前好市多/Costco 候選共 64 筆，每類最多 3 筆。
- 本輪更新 18 筆既有商品現價（11 筆降價、7 筆漲價），購買連結與既有圖片連結均不需更換；Tefal Daisy 28cm IH 的 LBJ 可重現史低由 NT$899 下修至 NT$699。momo 明示售完的 MC-KC1W NT$8,880 與 13 筆 PChome Qty 0 僅保留追蹤，沒有把缺貨價寫回。
- 同輪納入台灣官方與 PChome 均有現貨的 IRIS PCF-CDP18TEC 2026 新品；循環扇增為 22 筆，最終共 667 筆且所有分類仍至少 20 筆。Acer RP722-12W 因仍標示預購且缺正式發售證據，先列後續追蹤；本輪未找到品牌官方明示停產的新證據。
- 667 張商品圖片皆可讀取，最終 438 筆 `found`、229 筆 `not_found`；ExchangeRate-API 更新至 2026-07-18 00:02 UTC，17 筆外幣商品已依新批次重算。
- 前一輪於 2026-07-18 07:03（台灣時間；2026-07-17 23:03 UTC）以 664 筆基準清單全量重查價格、購買連結、圖片、438 筆 `found` 史低來源、匯率與原廠停產狀態。台灣商品優先台灣可信通路，17 筆海外商品才比較海外通路；好市多補充資料查核日同步為 2026-07-18，目前好市多/Costco 候選共 64 筆，每類最多 3 筆。
- 本輪在 07:00 促銷切換後再次確認正式價格，更新 102 筆既有商品現價（97 筆降價、5 筆漲價）、4 筆購買連結、4 張同型號來源圖片與 14 筆可公開驗證新史低。Mercusys MR27BE 的失效 PChome 頁改為同型號現貨頁，價格更新為 NT$2,299，並補正 EasyMesh 規格。
- 同輪納入 Samsung MRA65R95HAXXZW、LG WD-S1818PB 與 Samsung LS40H850TACXZW；HERAN HDF-16S6 因原商品頁失效且查無合規 exact-model 新品來源而移除，但禾聯官方產品頁仍可開啟，因此沒有宣稱官方停產。最終共 666 筆，電視 27 筆、洗烘衣機 24 筆、電腦螢幕 55 筆、電風扇 22 筆，所有分類仍至少 20 筆。
- 666 張商品圖片皆可讀取，最終 438 筆 `found`、228 筆 `not_found`；13 筆 PChome Qty 0 只保留追蹤。未找到品牌官方明示停產的新證據；ExchangeRate-API 仍是 2026-07-17 00:02 UTC 同一批次，17 筆外幣換算無差異。
- 前一輪於 2026-07-17 12:49（台灣時間；2026-07-17 04:49 UTC）以 664 筆清單全量重查價格、購買連結、圖片、437 筆 `found` 史低來源與原廠停產狀態；更新 66 筆價格（5 筆降價、61 筆漲價）、5 筆購買連結、2 張同型號來源圖片與 3 筆可公開驗證新史低。Panasonic F-H14GND-K 的失效 momo 頁改為 PChome 同型號現貨頁，現價與史低下修至 NT$2,990；Samsung RT22M4015S8/TW 改用 Yahoo 自營同型號現貨頁，現價與史低下修至 NT$11,900；SHARP FU-J41T-W 新增 NT$5,888 可重現史低。664 張圖片皆可讀取，最終 438 筆 `found`、226 筆 `not_found`；14 筆 PChome Qty 0 與 Yahoo 明示售罄商品只保留追蹤。
- 匯率仍為 ExchangeRate-API 2026-07-17 00:02 UTC 同一發布批次：USD/TWD 32.247891、GBP/TWD 43.492011、EUR/TWD 36.918781、JPY/TWD 0.198655、CNY/TWD 4.756088；17 筆外幣商品重算均無差異。
- 前一輪於 2026-07-17 07:55（台灣時間；2026-07-16 23:55 UTC）以原有 662 筆清單全量重查價格、購買連結、圖片、史低來源與停產狀態，納入 2 款官方新品後增為 664 筆。
- 2026-07-17 定期查核更新 77 筆價格（61 筆降價、16 筆漲價）、1 筆購買連結與 3 筆可公開驗證新史低；Airmate FB2391DR 改用品牌官方現貨頁，Whirlpool WDEE1200TW、BenQ BSH01、Samsung HW-Q930H/ZW 的史低分別下修至 NT$7,690、NT$2,999、NT$44,910。ASUS XG27AQDMES 的原 NT$10,343 來源已失去商品內容且無合規替代證據，改為 `not_found`。
- 同輪納入 ZWILLING UNLOCK 1034435 台灣新品與 Ninja MW101 海外參考；總數 664，鍋具與多功能氣炸烤箱／微波爐各 24 筆，所有分類仍至少 20 筆。664 張圖片皆可讀取，最終 437 筆 `found`、227 筆 `not_found` 史低；14 筆 PChome Qty 0 只保留追蹤，沒有新增原廠停產商品。匯率已更新至 2026-07-17 00:02 UTC，17 筆外幣商品已依新批次重算。
- 2026-07-16 夜間定期查核更新 3 筆價格，皆為漲價：WMF Durado 24cm NT$1,690 → NT$2,690、WMF Belmonte 20cm NT$1,869 → NT$2,288、Sony HT-S40R NT$8,193 → NT$8,490；購買連結與圖片連結無需更換，662 張圖片皆可讀取，438 筆 `found` 史低沒有新低或失效證據。依 HCG 官方頁明示移除已停產 AF890AW，納入有北美官方現貨、官方型號 HHR32F、上市與 exact-model 負評覆核證據的 Dreame Aero Ultra Steam 海外參考；總數維持 662，免治馬桶 20 筆、吸塵器 24 筆，各分類仍至少 20 筆。匯率仍為 2026-07-16 00:02 UTC 同一批次，16 筆外幣商品已重算；13 筆 PChome Qty 0 僅保留追蹤，沒有其他原廠停產或需立即納入的新產品。
- 2026-07-16 晚間定期查核更新 62 筆價格（5 筆降價、57 筆漲價）；購買連結無需更換，662 張圖片皆可讀取。LG DFB533FW 的 LBJ 可公開驗證史低由 NT$29,900 下修至 NT$28,255；其餘 437 筆 `found` 史低沒有需要失效或下修的證據。匯率仍是 2026-07-16 00:02 UTC 同一發布批次，15 筆外幣換算無差異；13 筆 PChome Qty 0 僅保留追蹤。未找到新的原廠停產證據或需立即納入的新品，各分類仍至少 21 筆、總數維持 662。
- 2026-07-16 定期查核更新 47 筆既有商品價格（36 筆降價、11 筆漲價）、1 筆購買連結與 1 張同型號官方圖片；Dyson Spot+Scrub Ai 改回台灣 Dyson 官方同型號現貨頁與 NT$24,900 台灣售價，Panasonic F-S14KM 因原 momo 史低頁失去可重現的商品與價格證據而改為 `not_found`。新增 LG OLED65C6PTA、Sony Y-65XR90M2、Samsung QA65S95HAXXZW 與 Sony HT-A7100，四款皆有台灣可信新品通路、官方發售證據、可驗證史低與 exact-model 跨站負評覆核；最終 438 筆 `found`、224 筆 `not_found` 史低，662 張圖片皆可讀取。13 筆 PChome Qty 0 僅保留追蹤，未找到新的原廠停產證據，各分類仍至少 21 筆。
- 2026-07-15 晚間定期查核更新 46 筆價格（28 筆降價、18 筆漲價）、2 筆購買連結與 1 筆可公開驗證史低；658 張商品圖片皆可讀取，435 筆 `found` 史低沒有需要失效的證據。Sony HT-S100F 改用同一 Yahoo 商品編號的 canonical URL；ASUS ROG Strix XG27AQDMES 改用 momo 同型號現貨頁；Samsung WW12BB744DGBTW 現價與史低下修為 NT$15,000。匯率仍是 2026-07-15 00:02 UTC 同一發布批次，16 筆外幣換算無差異；13 筆 PChome Qty 0 僅保留追蹤。未找到新的原廠停產證據或需立即納入的新品，各分類仍至少 21 筆、總數維持 658。
- 2026-07-15 定期查核更新 67 筆價格（8 筆降價、59 筆漲價）、1 筆購買連結、1 張同型號來源圖片、16 筆海外商品 TWD 換算與 2 筆可公開驗證史低。Sonos Arc Ultra 改用 K.Wild 同型號公司貨現貨頁，現價與史低為 NT$37,710；Acerpure KW534-60W 的 LBJ 歷史曲線新低為 NT$15,031。658 張圖片重試後皆可讀取，435 筆 `found` 史低沒有需要失效的證據；13 筆 PChome Qty 0 僅保留追蹤。未找到新的原廠停產證據或需立即納入的新品，25 類仍各至少 21 筆、總數維持 658。
- 2026-07-14 晚間定期查核更新 94 筆價格（51 筆降價、43 筆漲價）、13 筆購買連結與 1 張同型號白色款圖片，並下修 11 筆可公開驗證史低；660 張原始圖片皆可讀取。Samsung HW-Q990F 的 NT$42,914 原 PChome 史低來源失效且無合規替代證據，已改為 `not_found`，最終為 435 筆 `found`、223 筆 `not_found`。依原廠頁明確標示移除 BALMUDA EGF-3300 與 IRIS TAF-MKM10，循環扇仍有 21 筆；新品搜尋無需立即納入者，各分類仍至少 20 筆。
...
- 外幣匯率來源目前為 ExchangeRate-API；最新批次為 2026-07-19 00:02 UTC：USD/TWD 32.34116、GBP/TWD 43.4820417810595、EUR/TWD 36.98296603269571、JPY/TWD 0.19915457788164154、CNY/TWD 4.763618688505273；17 筆外幣商品已同步重算。

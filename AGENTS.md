...
- 網站由現有 37 類資料產生 37 個 `/categories/<id>/` 靜態分類指南頁；不建立 1156 個重複商品事實的薄內容頁。
...
- 特殊分類目前數量：
  ...
  - `電腦螢幕`：64 筆，需涵蓋主流品牌、不同用途帶與 34/40/45/49/57 吋寬螢幕。
  ...
  - `鍋具`：28 筆、`刀具`：23 筆、`櫥下飲水機`：43 筆、`洗碗機`：31 筆，需排除配件、耗材、桌上/直立誤判、福利品與展示機。
  - `冰箱`：27 筆、`洗衣機`：27 筆、`烘衣機`：24 筆、`洗烘衣機(多功能型)`：32 筆，需排除配件、耗材、福利品、展示機、箱損品、組合包誤判與單功能錯類，且每筆需標示機身尺寸。
...
- 櫥下飲水機已納入水空間 WS-RCH601、WS-Min'O-R2、WS-UVN500；完整機組與濾材／龍頭等配件分清楚，不以套組或顏色重複計數。
...
- `npm run check:geo`：37 個分類頁、1156 款靜態型號索引、metadata、結構化資料、首頁分類入口、sitemap、llms、六份公開證據檔、Pages artifact、38 個 canonical URL 的 IndexNow contract 與產生結果均無漂移。

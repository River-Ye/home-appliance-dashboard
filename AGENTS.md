...
- 特殊分類目前數量：
  - `掃拖機器人`：29 筆，需涵蓋知名品牌旗艦與次旗艦。
  - `無線路由器`：46 筆，只收 Wi-Fi 6 以上，Mesh/多 AP/商用管理支援要寫清楚。
  - `電腦螢幕`：52 筆，需涵蓋主流品牌、不同用途帶與 34/45/49/57 吋寬螢幕。
  - `懸臂支架`：21 筆，需標示支援尺寸、承重重量與是否適合 49/57 吋大寬螢幕。
  - `電子鎖`：46 筆，需涵蓋知名品牌且注意安裝條件。
  - `鍋具`、`刀具`、`櫥下飲水機`、`洗碗機`：各 21 筆，需排除配件、耗材、桌上/直立誤判、福利品與展示機。
  - `冰箱`、`洗衣機`、`烘衣機`、`洗烘衣機(多功能型)`：各 21 筆，需排除配件、耗材、福利品、展示機、箱損品、組合包誤判與單功能錯類。
  - `空氣清淨機`：21 筆，已補入 POIEMA 新氣几系列；後續若替換資料，除非無可信新品通路，需保留 POIEMA 候選。
  - `電風扇`：21 筆，已補入 Philips 風扇/循環扇/塔扇/無葉片款；後續若替換資料，除非無可信新品通路，需保留 Philips 候選。
...
- 頁面目前標示主要資料擷取日為 2026-07-09；好市多補充資料查核日為 2026-07-09。

...
- 網站由現有 26 類資料產生 26 個 `/categories/<id>/` 靜態分類指南頁；不建立 705 個重複商品事實的薄內容頁。
...
- 特殊分類目前數量：
  - `電視`：32 筆，其中 5 筆為 exact 70 吋，需涵蓋主流顯示技術與可信台灣新品通路。
  ...
  - `鍋具`：25 筆、`刀具`：21 筆、`櫥下飲水機`：21 筆、`洗碗機`：24 筆，需排除配件、耗材、桌上/直立誤判、福利品與展示機。
  - `冰箱`：24 筆、`洗衣機`：23 筆、`烘衣機`：21 筆、`洗烘衣機(多功能型)`：27 筆，需排除配件、耗材、福利品、展示機、箱損品、組合包誤判與單功能錯類，且每筆需標示機身尺寸。
  ...
  - `循環扇`：23 筆，已納入有台灣官方現貨、上市年份與 exact-model 查核證據的 IRIS PCF-CDP18TEC 與 SHARP PK-18S03T。

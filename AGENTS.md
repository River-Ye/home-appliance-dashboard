...
- 網站由現有 27 類資料產生 27 個 `/categories/<id>/` 靜態分類指南頁；不建立 743 個重複商品事實的薄內容頁。
...
- 特殊分類目前數量：
  ...
  - `無線路由器`：50 筆，只收 Wi-Fi 6 以上，Mesh/多 AP/商用管理支援要寫清楚。
  ...
  - `冰箱`：24 筆、`洗衣機`：23 筆、`烘衣機`：21 筆、`洗烘衣機(多功能型)`：28 筆，需排除配件、耗材、福利品、展示機、箱損品、組合包誤判與單功能錯類，且每筆需標示機身尺寸。

...
- 網站由現有 26 類資料產生 26 個 `/categories/<id>/` 靜態分類指南頁；不建立 714 個重複商品事實的薄內容頁。
...
- 2026-08-01 06:54（台灣時間）完成 714 筆商品、26 類全量查核；408 筆 PChome SKU API 已完成（262 筆 exact model、37 筆人工 SKU 綁定可寫入，82 筆型號未自動確認），更新 92 筆現價（47 筆降價、45 筆漲價），23 筆 Qty 0 只列追蹤。
- 新增 purifier-philips-ac0921-84；沒有移除產品。逐類人工新品覆核已完成，所有分類至少 20 筆；停產只採品牌官方明確證據，缺貨、反爬或單次連線錯誤不作為刪除依據。
- 圖片查核覆蓋 714 筆；560 筆來源可由 exact model 或人工 SKU 綁定確認，154 筆來源／圖片例外保留原資料。史低為 443 筆 `found`、271 筆 `not_found`，本輪下修 3 筆、其他更正 3 筆；58 筆來源可自動重現，其餘保留原逐筆證據且未臆測失效。
- ExchangeRate-API 最新批次為 2026-07-31 00:02 UTC，USD/TWD 32.37821；30 筆外幣商品已重算。完整摘要與例外保存在 `catalog_maintenance_latest.json`。

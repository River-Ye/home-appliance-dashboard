...
- The site has one generated static guide for every configured category (currently 37) at `/categories/<id>/`; do not create one thin page per product or add tracking for GEO work.
...
- Keep the complete 37-category × 9-brand `japaneseBrandReview` matrix in `catalog_maintenance_latest.json`; full maintenance requires same-date review, while an explicit `added_products_only` batch retains unchanged baseline category rows and their real dates. For the original +90 bedding batch, only 27 reviewed cells were appended and the baseline 306 retained; this historical scope does not replace a later explicitly requested full review. Defaults for unknown categories are not research evidence. Do not create a seventh public evidence file or relabel carried evidence as newly reviewed.
...
- Keep `mouse`, `keyboard`, and `mousepad` at exactly 30 each, after `monitorarm` in 工作網路, with `value/mid/premium` counts 6/12/12 and both office/gaming recommendations. Extend existing type filters only: mouse `standard/vertical/trackball`, keyboard `membrane/scissor/mechanical/magnetic/optical`, pad `cloth/hard/glass`. Require one Taiwan-warranty Top Pick per category; disclose no-warranty products honestly and do not count them as Taiwan-warranty Japanese representatives.
- For keyboards/pads, require `variantFamily`, at most three independently verified switch/planar-size variants per family, official full model/SKU, and a restorable exact-variant buy URL. Color, language, backlight, thickness, or bundles alone do not add variants. Exclude barebones, keycaps/switches, numpads, touchpads, replacement parts, and non-new goods; do not introduce a generic variant framework.
- In keyboard `軸體：` specs, put the full official switch identity before the first semicolon and all descriptive notes after it; duplicate-variant checks ignore those notes.
- Peripheral dimensions/net weights use mm/g and source-labeled axis order, with the same validator for product specs and evidence rows; legacy appliances retain cm/kg. Record pad thickness separately and require two explicitly sourced planar dimensions for size variants. Unknown fields use 查不到 with actual checked sources/reasons; do not transfer launch, historical-low, or issue evidence across variants. Follow the ordered spec prefixes in `PERIPHERAL_SPEC_PREFIXES`.
- Keep `bedsheet`, `comforter`, and `pillow` at exactly 30 each in the standalone 臥室寢具 group after 洗衣家務, with `value/mid/premium` counts 6/12/12 and exactly one Taiwan-channel Top Pick with a concrete local official/retailer after-sales route per category. Use only the approved material/fill `BEDDING_TYPES` and ordered `BEDDING_SPEC_PREFIXES`; do not add another filter dimension.
- Bedding products require `variantFamily` and at most three independently verified variants per family. Bedsheets/comforters may vary only by official size SKU; pillows may vary only by official height/firmness SKU. Colors, patterns, pack counts, gifts, and bundles alone do not add variants. Preserve exact centimeter dimensions and kilogram net weight evidence; comforter fill weight is separate from whole-product net weight, and bedsheet pocket height is separate from planar size.
- Bedsheets may be standalone fitted sheets or fitted-sheet-plus-pillowcase sets, but exclude duvet-cover sets. Comforters include cotton, down, synthetic, wool, and silk finished inserts/products while excluding covers, blankets, electric blankets, and cooling throws. Pillows are adult sleep pillows only; exclude cushions, bolsters, travel/infant pillows, cases, and replacement fill.

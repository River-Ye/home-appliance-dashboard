...
- When the original page contradicts the API, manually record the page's preorder status or expired/unreproducible discount conditions before finalizing. Do not apply an expired promotion or label a preorder as current stock. Yahoo promotions expire at their actual `endTs` instant, not at midnight on the same calendar date.
...
- Review Sony, Panasonic, HITACHI, Mitsubishi Electric, Daikin, GENERAL (Fujitsu General), Rinnai, Noritz, and TOTO across all 37 categories on every finalized full-maintenance date.
- Run `npm run review:japanese-brands -- --date=YYYY-MM-DD --baseline-ref=origin/main` after the full-maintenance draft. Every one of the 333 cells needs status, checked date, official sources, existing/added IDs, and a concrete reason. Under `added_products_only`, preserve unchanged baseline category rows and their original dates; the original bedding addition appended 27 reviewed cells while retaining 306. A later full-maintenance request requires a new review and explicit per-category limits for inaccessible sources or unconfirmed successor mappings. Never let a fallback regenerate all categories or treat a missing-map default as a reviewed decision. Keep this matrix in the committed maintenance report rather than a seventh public JSON.
...
- Keep `dimension_research.json` aligned with the 21 dimension categories, including the existing appliance categories plus mouse, keyboard, mousepad, bedsheet, comforter, and pillow. The same file carries researched weight evidence for 13 categories: the existing seven weight categories plus those six peripheral and bedding categories.
...
- Bedsheets, comforters, and pillows use source-labeled cm/kg measurements. Bedsheets also record pocket height, comforters keep fill weight separate from whole-product net weight, and pillows record the selected official height/firmness SKU. Missing values use 查不到 with actual checked sources and reasons; never use packaging measurements or gross weight.

## Bedding Scope

- `bedsheet`, `comforter`, and `pillow` each contain exactly 30 products with value/mid/premium counts 6/12/12, all in the 臥室寢具 group after 洗衣家務. Keep one Taiwan Top Pick in each category and scenario tags for cooling/skin comfort, four-season/winter warmth, and back/side sleeping.
- Bedsheets cover cotton, lyocell, linen, synthetic, and other natural fibers; allow a fitted sheet alone or a fitted-sheet-plus-pillowcase set, but exclude duvet-cover sets. Comforters cover cotton, down, synthetic, wool, and silk finished inserts/products, excluding covers, blankets, electric blankets, and cooling throws. Pillows cover latex, memory foam, down/feather, fiber, and hybrid adult sleep pillows, excluding cushions, bolsters, travel/infant pillows, cases, and replacement fill.
- Require `variantFamily` and at most three independently verified variants per family. Bedsheet/comforter variants require distinct official size SKUs; pillow variants require distinct official height/firmness SKUs. Colors, patterns, pack counts, gifts, and bundles alone do not create variants. Keep exact-variant buy URLs and independent price, image, release, historical-low, dimension/weight, and issue evidence.

...
- Run `npm run review:japanese-brands -- --date=YYYY-MM-DD --baseline-ref=origin/main` after the full-maintenance draft. Every one of the 333 cells needs status, checked date, official sources, existing/added IDs, and a concrete reason. Under `added_products_only`, preserve unchanged baseline category rows and their original dates; the bedding addition appends 27 reviewed cells while retaining 306. Never let a fallback regenerate all categories or treat a missing-map default as a reviewed decision. Keep this matrix in the committed maintenance report rather than a seventh public JSON.

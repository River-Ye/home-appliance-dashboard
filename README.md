...
純前端靜態頁面，整理 2026-09-12 查核的家電推薦清單，並保留好市多可購買候選，涵蓋價格、歷史最低價、負評／災情、入手時機、規格、優缺點、通路連結與海外價格 TWD 換算。
...
- 共 37 類商品，每種商品至少 20 個，共 1156 筆。
- 另由現有分類與商品資料產生 37 個可獨立索引的 `/categories/<id>/` 選購指南頁；每頁的初始 HTML 均含前 5 名詳細推薦與該類全部型號索引，不為 1156 筆商品建立重複、薄內容的獨立頁面。
...
- 掃拖機器人共 45 款；新增 MOVA Z70 Ultra Roller Complete 台灣 R5765U，核對 110V／50–60Hz、兩年保固與現售價格，明示組合台階越障及海外個別回報限制。保留既有 S70 與其他型號；E20 依先前官方停止生產公告移除。
- 洗衣機共 27 款；新增 Panasonic NA-V150SW 15kg 前開溫水單洗款，補充不同容量與型態，保留既有直立與 12kg 滾筒機。
- 洗碗機共 31 款；新增 Haier W32／HDWE16-58GY3TW，明列去檯面後 76.5cm 機高、最低 76.8cm 開孔、220V／60Hz 與另計安裝費；淨重查不到，搬運前需確認。
...
- 電子衣櫥共 21 款；新增 Panasonic N-R600AR-X1 台灣 110V／60Hz 鏡面款，保留既有窄機身與海外型號。
- 吸塵器共 32 款；新增 HITACHI PV-XH4Q 並保留 PV-XH4P，明示新款的貼邊／防纏優勢與吸力、重量取捨。
- 櫥下飲水機共 43 款；新增水空間 WS-RCH601、WS-Min'O-R2、WS-UVN500，依完整型號分列常溫／多段熱水、直出 RO 與超濾 LED-UV 功能，保留既有 40 款比較。
...
- Sony、Panasonic、HITACHI、Mitsubishi Electric、Daikin、GENERAL、Rinnai、Noritz、TOTO 以 37 類 × 9 品牌完整矩陣保存人工盤點結果；只在有台灣合格 exact model 且確實增加比較價值時補代表款。完整維護重新盤點 333 格；僅新增批次保留基準與真實覆核日期。品牌已有商品不代表全部世代已核實，來源阻擋與未確定後繼的缺口另列於逐類紀錄。
...
- 支援分類、冷氣／熱水器型態、網路交換器 1G／2.5G／10G 速率、滑鼠握持型態、鍵盤機構、滑鼠墊與寢具材質／填充、品牌、選購定位、通路、自然多詞搜尋、排序、active filter chips、可分享 URL 篩選狀態與比較清單；既有 `budget` query key 維持相容。
- 首屏與 37 個分類指南皆清楚標示 AI 協作、資料日期、查核方法與 GitHub 原始碼；資料日代表最近完成的研究批次，個別商品查核日期以逐筆證據為準，不等於全目錄均於當日重查。網站與內容仍可能有錯漏，不虛構專家或實機使用經驗。
...
- 2026-09-12 20:47（台灣時間）完成 1156 筆商品、37 類全量查核；503 筆 PChome SKU API 已完成（265 筆 exact model、66 筆人工 SKU 綁定可寫入，73 筆型號未自動確認），更新 65 筆公開價格（24 筆下修、41 筆上修），51 筆 Qty 0 只列追蹤。
- 本次增量沒有納入新產品；本次增量沒有移除停產產品。沿用既有逐類逐型號證據，並補查本次官方入口（原證據日期保留），所有分類至少 20 筆；停產只採品牌官方明確證據，缺貨、反爬或單次連線錯誤不作為刪除依據。
- 圖片查核覆蓋 1156 筆；841 筆來源可由 exact model 或人工 SKU 綁定確認，315 筆來源例外與 1 筆圖片例外保留原資料。史低為 509 筆 `found`、647 筆 `not_found`，本輪下修 5 筆、其他更正 5 筆；84 筆來源可自動重現，其餘保留原逐筆證據且未臆測失效。
- ExchangeRate-API 最新批次為 2026-09-12 00:02 UTC，USD/TWD 31.577975；38 筆外幣商品已重算。完整摘要與例外保存在 `catalog_maintenance_latest.json`。
...
- 每個分類頁會從同一份商品資料產生可直接擷取的「完整型號索引」，讓無 JavaScript 爬蟲也能讀到全部 1156 款的品牌、型號、名稱、參考價、上市日期與摘要。
...
- `npm run check:data`：檢查 37 類、1156 筆、必要欄位、日期格式、21 類尺寸與 13 類新增重量規格／證據對齊、鍵鼠與寢具各類 30 款及 6／12／12、同款最多三版、型態與版本重複、螢幕燈 20 款、網路交換器 20 款、冷氣 30 組、熱水器 45 台、333 格日系品牌矩陣、價格／安裝 enum、歷史最低價與負評 research 對齊、人工覆核完成清單、逐位反映者、6 人／2 平台門檻、每類至少 20 筆、重複型號與重複購買 URL。
- `npm run check:docs`：檢查 README、AGENTS、index/config 的商品數、分類數、日期與 cache version 沒有漂移。
- `npm run check:geo`：檢查 37 個分類頁、1156 款靜態型號索引、metadata、結構化資料、內部連結、sitemap、llms、證據檔、Pages artifact、38 個 canonical IndexNow URL contract 與所有產生結果沒有漂移。
...
- `npm run review:japanese-brands -- --date=YYYY-MM-DD --baseline-ref=origin/main`：全量維護時在 draft 上寫入 37 類 × 9 品牌同日人工盤點矩陣；僅新增模式不得重寫沿用分類，矩陣不另建第七份公開證據檔。

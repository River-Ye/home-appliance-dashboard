...
純前端靜態頁面，整理 2026-07-16 查核的家電推薦清單，並保留好市多可購買候選，涵蓋價格、歷史最低價、負評／災情、入手時機、規格、優缺點、通路連結與海外價格 TWD 換算。
...
- 共 25 類商品，每種商品至少 20 個，共 662 筆。
- 另由現有分類與商品資料產生 25 個可獨立索引的 `/categories/<id>/` 選購指南頁；不為 662 筆商品建立重複、薄內容的獨立頁面。
...
- 662 筆商品皆有同型號負評／災情查核；2026-07-16 新增的 4 款商品皆已用包含完整 canonical model 的 Mobile01 與 Reddit 查詢逐頁覆核，未找到 exact-model 候選。2026-07-11 重查的 612 筆原先完全沒有負面候選商品、36 筆既有集中負評與 13 筆已有少量負面證據者則保留原研究內容。只有同一問題由至少 6 位可辨識的獨立使用者反映、且跨至少 2 個原始網站時才顯示紅色警示；未達門檻統一顯示「截至查核日，查無達門檻的集中負評／災情」。
...
- 海外價格使用 ExchangeRate-API 最新回傳的 2026-07-16 00:02 UTC 匯率換算：USD/TWD 32.190284、GBP/TWD 43.367994、EUR/TWD 36.855151、JPY/TWD 0.198515、CNY/TWD 4.749024；15 筆外幣商品已同步重算，仍未含國際運費、進口稅與台灣保固。
...
- `npm run check:data`：檢查 25 類、662 筆、必要欄位、日期格式、四類大型家電尺寸、歷史最低價與負評 research 對齊、人工覆核完成清單、逐位反映者、6 人／2 平台門檻、每類至少 20 筆、重複型號與重複購買 URL。

...
- 2026-08-20 06:55（台灣時間）完成 859 筆商品、29 類全量查核；429 筆 PChome SKU API 已完成（263 筆 exact model、45 筆人工 SKU 綁定可寫入，81 筆型號未自動確認），更新 0 筆公開價格（0 筆下修、0 筆上修），28 筆 Qty 0 只列追蹤。
- 本次增量沒有納入新產品；本次增量沒有移除停產產品。沿用本資料日已完成的逐類人工新品覆核（原覆核時間保留），所有分類至少 20 筆；停產只採品牌官方明確證據，缺貨、反爬或單次連線錯誤不作為刪除依據。
- 圖片查核覆蓋 859 筆；638 筆來源可由 exact model 或人工 SKU 綁定確認，223 筆來源／圖片例外保留原資料。史低為 468 筆 `found`、391 筆 `not_found`，本輪異動 0 筆；73 筆來源可自動重現，其餘保留原逐筆證據且未臆測失效。

...
純前端靜態頁面，整理 2026-09-13 查核的家電推薦清單，並保留好市多可購買候選，涵蓋價格、歷史最低價、負評／災情、入手時機、規格、優缺點、通路連結與海外價格 TWD 換算。
...
- 2026-09-13 08:20（台灣時間）完成 1156 筆商品、37 類全量查核；503 筆 PChome SKU API 已完成（265 筆 exact model、66 筆人工 SKU 綁定可寫入，73 筆型號未自動確認），更新 21 筆公開價格（2 筆下修、19 筆上修），51 筆 Qty 0 只列追蹤。
- 本次增量沒有納入新產品；本次增量沒有移除停產產品。沿用既有逐類逐型號證據，並補查本次官方入口（原證據日期保留），所有分類至少 20 筆；停產只採品牌官方明確證據，缺貨、反爬或單次連線錯誤不作為刪除依據。
- 圖片查核覆蓋 1156 筆；841 筆來源可由 exact model 或人工 SKU 綁定確認，315 筆來源例外與 1 筆圖片例外保留原資料。史低為 509 筆 `found`、647 筆 `not_found`，本輪其他更正 5 筆；148 筆來源可自動重現，其餘保留原逐筆證據且未臆測失效。
- ExchangeRate-API 最新批次為 2026-09-13 00:02 UTC，USD/TWD 31.564695；38 筆外幣商品已重算。完整摘要與例外保存在 `catalog_maintenance_latest.json`。

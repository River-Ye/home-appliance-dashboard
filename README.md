...
- 2026-07-22 08:42（台灣時間；2026-07-22 00:42 UTC）完成 668 筆商品複核。修正 PChome 現價規則後，相較 2026-07-21 基準清單共有 33 筆既有商品異動（23 筆降價、10 筆漲價）；401 筆 PChome exact-model 頁已重查，18 筆 Qty 0 只列追蹤，未把缺貨價寫回。
- 本輪保留已納入的 Dyson HJ10 空氣清淨機與 LG WD-S2220B 22/20kg WashTower，並補正 HJ10 官方限時價、購買頁、圖片與正式發售日，以及 WD-S2220B 官方發表日。總數維持 668 筆，25 類皆至少 20 筆；逐類複核未找到 2026-07-21 後真正新上市且需立即納入的商品。
- 668 張商品圖片皆可讀取；438 筆 `found` 史低來源中 335 筆可自動重現、55 筆被來源阻擋、48 筆無法自動確認 exact model，後兩者保留原逐筆證據且未臆測失效。本輪沒有可直接套用的品牌官方明示停產證據，3 筆文字掃描候選經原頁覆核皆為誤判。
- ExchangeRate-API 最新批次仍為 2026-07-21 00:02 UTC，USD/TWD 32.327985，沒有較新匯率可寫回。完整逐筆結果保存於 `catalog_maintenance_2026-07-22.json`。
...
- 668 筆現有商品皆有同型號負評／災情查核；IRIS PCF-CDP18TEC、BenQ PD2732U、Dyson HJ10 與 LG WD-S2220B 已以包含完整 canonical model 的跨站查詢逐站覆核。2026-07-11 重查的既有資料、36 筆集中負評與少量未達門檻證據均保留原研究內容。只有同一問題由至少 6 位可辨識的獨立使用者反映、且跨至少 2 個原始網站時才顯示紅色警示；未達門檻統一顯示「截至查核日，查無達門檻的集中負評／災情」。
...
- 海外價格使用 ExchangeRate-API 最新回傳的 2026-07-21 00:02 UTC 同一 USD 批次換算：USD/TWD 32.327985、GBP/TWD 43.42987780303396、EUR/TWD 36.91344724347951、JPY/TWD 0.19898050510862741、CNY/TWD 4.7694872179217604；17 筆外幣商品已同步重算，仍未含國際運費、進口稅與台灣保固。

...
- 共 37 類商品，每種商品至少 20 個，共 1149 筆。
- 另由現有分類與商品資料產生 37 個可獨立索引的 `/categories/<id>/` 選購指南頁；每頁的初始 HTML 均含前 5 名詳細推薦與該類全部型號索引，不為 1149 筆商品建立重複、薄內容的獨立頁面。
...
- 2026-09-07 17:31（台灣時間）完成 1149 筆商品、37 類全量查核；500 筆 PChome SKU API 已完成（267 筆 exact model、65 筆人工 SKU 綁定可寫入，74 筆型號未自動確認），更新 21 筆公開價格（7 筆下修、14 筆上修），53 筆 Qty 0 只列追蹤。
- 本次增量新增 dehumidifier-whirlpool-wdee123htw、monitor-msi-mpg271qr-x50；本次增量沒有移除停產產品。沿用本資料日既有逐類人工新品覆核，並補查部分分類（原覆核時間保留），所有分類至少 20 筆；停產只採品牌官方明確證據，缺貨、反爬或單次連線錯誤不作為刪除依據。
- 圖片查核覆蓋 1149 筆；839 筆來源可由 exact model 或人工 SKU 綁定確認，310 筆來源例外與 0 筆圖片例外保留原資料。史低為 508 筆 `found`、641 筆 `not_found`，本輪異動 0 筆；84 筆來源可自動重現，其餘保留原逐筆證據且未臆測失效。
...
- 每個分類頁會從同一份商品資料產生可直接擷取的「完整型號索引」，讓無 JavaScript 爬蟲也能讀到全部 1149 款的品牌、型號、名稱、參考價、上市日期與摘要。
...
- `npm run check:data`：檢查 37 類、1149 筆、必要欄位、日期格式、21 類尺寸與 13 類新增重量規格／證據對齊、鍵鼠與寢具各類 30 款及 6／12／12、同款最多三版、型態與版本重複、螢幕燈 20 款、網路交換器 20 款、冷氣 30 組、熱水器 45 台、333 格日系品牌矩陣、價格／安裝 enum、歷史最低價與負評 research 對齊、人工覆核完成清單、逐位反映者、6 人／2 平台門檻、每類至少 20 筆、重複型號與重複購買 URL。
- `npm run check:docs`：檢查 README、AGENTS、index/config 的商品數、分類數、日期與 cache version 沒有漂移。
- `npm run check:geo`：檢查 37 個分類頁、1149 款靜態型號索引、metadata、結構化資料、內部連結、sitemap、llms、證據檔、Pages artifact、38 個 canonical IndexNow URL contract 與所有產生結果沒有漂移。

...
- Sony、Panasonic、HITACHI、Mitsubishi Electric、Daikin、GENERAL、Rinnai、Noritz、TOTO 以 31 類 × 9 品牌完整矩陣保存人工盤點結果；只在有台灣合格 exact model 且確實增加比較價值時補代表款。增量新增時保留原分類真實覆核日期，不宣稱全部分類於本輪重查。
...
- 首屏與 31 個分類指南皆清楚標示 AI 協作、資料日期、查核方法與 GitHub 原始碼；資料日代表最近完成的研究批次，個別商品查核日期以逐筆證據為準，不等於全目錄均於當日重查。網站與內容仍可能有錯漏，不虛構專家或實機使用經驗。
...
- `tools/category-guides.js` 是 31 類導讀、選購重點與 FAQ 的人工維護來源；商品事實仍以 `assets/js/config.js` 與 `products/*.js` 為準。
- 每個分類頁會從同一份商品資料產生可直接擷取的「完整型號索引」，讓無 JavaScript 爬蟲也能讀到全部 943 款的品牌、型號、名稱、參考價、上市日期與摘要。
...
- `npm run check:data`：檢查 31 類、943 筆、必要欄位、日期格式、15 類尺寸與 7 類新增重量規格／證據對齊、螢幕燈 20 款與 13 項規格前綴、網路交換器 20 款與三速／八埠／非 PoE 契約、冷氣 30 組與型態／坪數帶／室內外 pair、熱水器 45 台與 15／15／15、電熱 8／7、279 格日系品牌矩陣、價格／安裝 enum、電子衣櫥與咖啡機契約、歷史最低價與負評 research 對齊、人工覆核完成清單、逐位反映者、6 人／2 平台門檻、每類至少 20 筆、重複型號與重複購買 URL。
- `npm run check:docs`：檢查 README、AGENTS、index/config 的商品數、分類數、日期與 cache version 沒有漂移。
- `npm run check:geo`：檢查 31 個分類頁、943 款靜態型號索引、metadata、結構化資料、內部連結、sitemap、llms、證據檔、Pages artifact、32 個 canonical IndexNow URL contract 與所有產生結果沒有漂移。
...
- `npm run review:japanese-brands -- --date=YYYY-MM-DD --baseline-ref=origin/main`：全量維護時在 draft 上寫入 31 類 × 9 品牌同日人工盤點矩陣；僅新增模式不得重寫沿用分類，矩陣不另建第七份公開證據檔。

...
- 2026-07-22 07:57（台灣時間）以 666 筆清單全量查核 400 筆 PChome API、266 筆其他購買連結、611 次圖片回應、437 筆 `found` 史低來源與匯率；更新 77 筆 PChome exact-model 現貨價（2 筆降價、75 筆漲價），未更換圖片連結。17 筆 PChome Qty 0 僅保留追蹤，沒有把缺貨價寫回。
- 購買連結 6 筆、圖片 0 筆、史低來源 46 筆未能在本輪取得成功回應，均保留原資料並列入後續人工覆核，不以單次網路錯誤判定停產；偵測到 4 筆頁面含停產字樣候選，但本輪未自動移除，需以原廠明確證據人工覆核。本輪沒有納入資料證據不足、預購、福利品、展示機、配件或不同型號候選，也沒有移除既有產品。
- 最終共 666 筆、共 25 類，且各類皆至少 20 筆；史低狀態為 437 筆 `found`、229 筆 `not_found`。ExchangeRate-API 更新至 2026-07-21 00:02 UTC，17 筆外幣商品已依同一 USD 批次重算；USD/TWD 為 32.327985.

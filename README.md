...
純前端靜態頁面，整理 2026-08-17 查核的家電推薦清單，並保留好市多可購買候選，涵蓋價格、歷史最低價、負評／災情、入手時機、規格、優缺點、通路連結與海外價格 TWD 換算。
...
- 2026-08-17 08:41（台灣時間）完成 859 筆商品、29 類全量查核；429 筆 PChome SKU API 已完成（266 筆 exact model、43 筆人工 SKU 綁定可寫入，81 筆型號未自動確認），更新 59 筆公開價格（18 筆下修、41 筆上修），28 筆 Qty 0 只列追蹤。
- 本次增量沒有納入新產品；本次增量沒有移除停產產品。逐類人工新品覆核已完成，所有分類至少 20 筆；停產只採品牌官方明確證據，缺貨、反爬或單次連線錯誤不作為刪除依據。
- 圖片查核覆蓋 859 筆；662 筆來源可由 exact model 或人工 SKU 綁定確認，200 筆來源／圖片例外保留原資料。史低為 468 筆 `found`、391 筆 `not_found`，本輪下修 1 筆、其他更正 3 筆；70 筆來源可自動重現，其餘保留原逐筆證據且未臆測失效。
- ExchangeRate-API 最新批次為 2026-08-17 00:02 UTC，USD/TWD 31.966724；30 筆外幣商品已重算。完整摘要與例外保存在 `catalog_maintenance_latest.json`。
